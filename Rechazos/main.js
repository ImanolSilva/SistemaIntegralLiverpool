...
function slugify(text) { if (!text) return 'producto'; const a = 'àáâäæãåāăąçćčđďèéêëēėęěğǵḧîïíīįìłḿñńǹňôöòóœøōõőṕŕřßśšşșťțûüùúūǘůűųẃẍÿýžźż·/_,:;'; const b = 'aaaaaaaaaacccddeeeeeeeegghiiiiiilmnnnnoooooooooprrsssssttuuuuuuuuuwxyyzzz------'; const p = new RegExp(a.split('').join('|'), 'g'); return text.toString().toLowerCase().replace(/\s+/g, '-').replace(p, c => b.charAt(a.indexOf(c))).replace(/&/g, '-and-').replace(/[^\w\-]+/g, '').replace(/\-\-+/g, '-').replace(/^-+/, '').replace(/-+$/, ''); }
function loadDynamicImage(sku, seccion, containerId) { const container = document.getElementById(containerId); if (!container) return; container.innerHTML = `<div class="d-flex align-items-center justify-content-center" style="height: 150px;"><div class="spinner-border text-secondary" role="status"></div></div>`; const imageUrl = `https://ss${seccion}.liverpool.com.mx/lg/${sku}.jpg`; const fallbackImageHTML = `<div class="img-placeholder d-flex align-items-center justify-content-center p-3"><i class="bi bi-image-alt fs-1 text-muted"></i></div>`; const img = new Image(); img.src = imageUrl; img.className = "img-fluid rounded"; img.style.maxHeight = "200px"; img.onload = () => { container.innerHTML = ''; container.appendChild(img); }; img.onerror = () => { container.innerHTML = fallbackImageHTML; }; }
...
function setupEventListeners() {
    ...
    document.addEventListener("input", (e) => { if (e.target.classList.contains("comentario-input")) { const rowIndex = e.target.dataset.rowIndex; const newComment = e.target.value; if (AppState.rechazosGlobal[rowIndex]) { const remision = AppState.rechazosGlobal[rowIndex].Remisión; AppState.rechazosGlobal[rowIndex].Comentarios = newComment; queueChange(remision, 'Comentarios', newComment); } const hasComment = newComment.trim() !== ""; const indicator = document.querySelector(`#heading-${rowIndex} .state-indicator`); if(indicator) indicator.classList.toggle("has-comment", hasComment); if (autoSaveTimers[rowIndex]) clearTimeout(autoSaveTimers[rowIndex]); autoSaveTimers[rowIndex] = setTimeout(() => syncPendingChanges(true), 2000); } });
}
 ...
 *****************************************************/
function choosePhoto(rowIndex) { const fileInput = document.createElement('input'); fileInput.type = 'file'; fileInput.accept = 'image/*'; if (isMobile()) { fileInput.capture = 'environment'; } fileInput.onchange = (e) => { const file = e.target.files[0]; if (!file) return; const remision = AppState.rechazosGlobal[rowIndex]?.Remisión || `evidencia_${Date.now()}`; const filename = `uploads/evidencias/${remision}_${file.name.replace(/[^a-zA-Z0-9.]/g, '')}`; const storageRef = storage.ref(filename); Swal.fire({ title: 'Subiendo foto...', html: 'Por favor, espera.', allowOutsideClick: false, didOpen: () => { Swal.showLoading(); } }); const uploadTask = storageRef.put(file); uploadTask.on('state_changed', null, (error) => { console.error("Error al subir foto:", error); Swal.close(); showAlert("error", "Error al subir", "No se pudo guardar la evidencia."); }, () => { uploadTask.snapshot.ref.getDownloadURL().then(downloadURL => { AppState.rechazosGlobal[rowIndex].Fotos = downloadURL; updatePhotoPreview(rowIndex, downloadURL); queueChange(remision, 'Fotos', downloadURL); Swal.close(); showAlert('success', '¡Evidencia Guardada!', 'La foto se subió correctamente.'); }); }); }; fileInput.click(); }
...
function updatePhotoPreview(rowIndex, url) { const previewContainer = document.getElementById(`evidencia-preview-${rowIndex}`); if (!previewContainer) return; if (url && url.trim() !== "") { previewContainer.innerHTML = ` <div class="evidence-thumbnail-container"> <a href="${url}" target="_blank" title="Ver imagen completa"> <img src="${url}" alt="Evidencia" class="evidence-thumbnail"> </a> </div> <div class="evidence-actions"> <button class="btn btn-pill btn-pill-sm btn-photo" data-action="choose-photo" data-row-index="${rowIndex}"><i class="bi bi-camera-fill"></i> Cambiar</button> <button class="btn btn-pill btn-pill-sm btn-delete" data-action="delete-photo" data-row-index="${rowIndex}"><i class="bi bi-trash-fill"></i> Eliminar</button> </div>`; } else { previewContainer.innerHTML = ` <div class="evidence-placeholder" data-action="choose-photo" data-row-index="${rowIndex}"> <i class="bi bi-cloud-arrow-up-fill display-4 text-muted"></i> <span class="mt-2 fw-bold text-muted">Agregar Evidencia</span> </div>`; } }
...
function renderRechazoItem(rechazo, i) {
    ...
    const productoSlug = slugify(descripcionSku);
    const liverpoolUrl = `https://www.liverpool.com.mx/tienda/pdp/${productoSlug}/${Sku}?skuid=${Sku}`;

    return `
    <div class="accordion-item" id="item-${_rowIndex}">
        <h2 class="accordion-header" id="heading-${_rowIndex}">
            <button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-${_rowIndex}">
                <div class="badge-reporte">${Remisión}</div>
                <div class="report-summary-info flex-grow-1">
                    <div class="sku" title="${descripcionSku}">${descripcionSku}</div>
                    <div class="usuario"><i class="bi bi-person"></i> ${usuarioName}</div>
                </div>
                <div class="state-indicator ${hasComment ? 'has-comment' : ''}" title="${hasComment ? 'Tiene comentarios' : 'Sin comentarios'}"></div>
            </button>
        </h2>
                ...
                <div class="row g-4">
                    <div class="col-lg-7">
                        ...
                        <div class="report-details-grid">
                            <div class="detail-item"><i class="bi bi-upc-scan"></i><div class="detail-item-content"><small>SKU</small><span>${Sku}</span></div></div>
                            <div class="detail-item"><i class="bi bi-box-seam"></i><div class="detail-item-content"><small>Piezas</small><span>${piezas}</span></div></div>
                            <div class="detail-item"><i class="bi bi-calendar-event"></i><div class="detail-item-content"><small>Fecha</small><span>${fecha}</span></div></div>
                            <div class="detail-item"><i class="bi bi-tag-fill"></i><div class="detail-item-content"><small>Sección</small><span>${Sección}</span></div></div>
                            <div class="detail-item"><i class="bi bi-person-workspace"></i><div class="detail-item-content"><small>Jefatura</small><span>${jefatura}</span></div></div>
                        </div>
                        <div class="mt-4">
                    ...
                    <div class="col-lg-5">
                        <div class="evidence-card">
                            <h6 class="mb-3"><i class="bi bi-camera-fill me-2"></i>Evidencia Fotográfica</h6>
                            <div id="imgContainer-${Sku}-${i}" class="mb-3 product-image-container"></div>
